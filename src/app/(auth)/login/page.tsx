...
function LoginForm() {
  ...
  const [error, setError] = useState(() => {
    if (searchParams.get('error') === 'setup') {
      return 'Não foi possível configurar sua agência. Execute o migration.sql no Supabase e tente novamente.';
    }
    return '';
  });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FFF0E6] via-white to-[#F9FAFB] flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <Suspense fallback={<div className="bg-white rounded-2xl shadow-xl border border-[#F3F4F6] p-8 h-64 animate-pulse" />}>
          <LoginForm />
        </Suspense>
